...
# Usage: python extract_single_test_data.py -cwd run_kmeans/l1da_4_l2s_128kB_options_kdd_cup-100_l2a_1_l1ia_1_l1ds_16kB_cls_32_l1is_1kB_cpu_TimingSimpleCPU


stats = {
    "system.cpu.numCycles" : "cpu_cycles"
}
required_mcpat = [("Processor", "Peak Power", "System Peak Power"), 
...
def ReadYosysResult(yosys_file):
    ...
    with open(yosys_file, "r") as fh:
        ...
        number = re.compile(r'^\s*Number of (wires|wire bits|memories|memory bits|cells):\s*([^\s$]*)\s*$')
        enabled = False
        for line in lines:
            if comp.search(line):
                enabled = True
...
float_rx = re.compile('[-+]? (?: (?: \d* \. \d+ ) | (?: \d+ \.? ) )(?: [Ee] [+-]? \d+ ) ?', re.VERBOSE)
...
def main(args):
    ...
    print("System Leakage Power:"+str(GetProfileData(data_hash, "System Leakage Power")))
