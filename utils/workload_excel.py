...
class Workload:
    def __init__(self):
        self.param_exclude_re = re.compile(r'eval_id|^options_rows|^time$|^pid$|interface|index|machine|^cpu$|^CounterCycles$|WorkDir|area|power', re.IGNORECASE)
    ...
    def GetParamIndexes(self, hdrs):
        return [index for index, hdr in enumerate(hdrs) if not self.param_exclude_re.search(hdr) and not self.cost_re.search(hdr)]
    ...
    def GetValueNormAtIndexes(self, data, index, indexes, exclusions={}):
        ...
        return [unique_mean_values_hash[i][data[i]] if i not in exclusions else data[i] for i in indexes]
    ...
    def GetValueIndexAtIndexes(self, data, indexes, exclusions={}):
        ...
        return [unique_values_hash[i][data[i]] if i not in exclusions else data[i] for i in indexes]
        #return map(lambda i: unique_values_hash[i][data[i]], indexes)    
    ...
    def InitializeData(self, data_lines):
        ...
        self.unique_values = [list(set(self.GetSingleColumn(col))) for col, hdr in enumerate(self.headers_data)]
        self.max_min_values = []
        ...
        self.unique_mean_values_hash = [{val:self.GetNormValue(val, self.max_min_values[i]) for index,val in enumerate(hdata)} for i, hdata in enumerate(self.unique_values)]
        self.unique_values_hash = [{val:index for index,val in enumerate(hdata)} for hdata in self.unique_values]
    ...
    def ReadPreEvaluatedWorkloads(self, filename, count=-1):
        ...
        with open(filename) as fh:
            ...
            print("Params:"+str([str(index)+":"+self.headers_data[index] for index in self.param_indexes]))
            print("Cost:"+str([str(index)+":"+self.headers_data[index] for index in self.cost_indexes]))
    ...
    def ExcludeData(self, specific_columns, data_lines=None):
        ...
        if type(indexes[0]) == str or (sys.version_info[0] < 3 and  type(indexes[0]) == unicode):
            indexes = [self.headers_index_hash[col] for col in specific_columns.keys() if col in self.headers_index_hash]
    ...
    def Get2DDataWithIndexing(self, specific_columns, data_lines=None, exclusions={}):
        if data_lines == None:
            data_lines = self.data_lines
        if type(specific_columns[0]) == str or (sys.version_info[0] < 3 and  type(specific_columns[0]) == unicode):
            indexes = [self.headers_index_hash[col] for col in specific_columns if col in self.headers_index_hash]
            return [self.GetValueIndexAtIndexes(rdata, indexes, exclusions) for rdata in data_lines]
        return [self.GetValueIndexAtIndexes(rdata, specific_columns, exclusions) for rdata in data_lines]

    def Get2DDataWithNormalization(self, specific_columns, data_lines=None, exclusions={}):
        if data_lines == None:
            data_lines = self.data_lines
        if type(specific_columns[0]) == str or (sys.version_info[0] < 3 and  type(specific_columns[0]) == unicode):
            indexes = [self.headers_index_hash[col] for col in specific_columns if col in self.headers_index_hash]
            return [self.GetValueNormAtIndexes(rdata, index, indexes, exclusions) for index, rdata in enumerate(data_lines)]
        return [self.GetValueNormAtIndexes(rdata, index, specific_columns, exclusions) for index, rdata in enumerate(data_lines)]
    ...
    def Get2DLogData(self, specific_columns, data_lines=None):
        ...
        return [self.GetLogDataAtIndexes(rdata, specific_columns) for rdata in data_lines]
    ...
    def Get2DDataWithCheck(self, specific_columns, data_lines=None):
        ...
        if type(specific_columns[0]) == str or (sys.version_info[0] < 3 and  type(specific_columns[0]) == unicode):
            indexes = [self.headers_index_hash[col] for col in specific_columns if col in self.headers_index_hash]
            return [self.GetDataAtIndexesWithCheck(rdata, indexes) for rdata in data_lines]
        return [self.GetDataAtIndexesWithCheck(rdata, specific_columns) for rdata in data_lines]
    ...
    def WriteExcel(self, filename, headers=None, data=None):
        ...
        start_row = 0
        start_col = 0
        worksheet.add_table(start_row, start_col, start_row+rows-1, start_col+cols-1, 
                {'data': data,
                 'columns': hdr_list})
...
def StitchData(args):
    ...
    for k in group_data_keys.keys():
        ...
        for index, (workload, group_data, valid_hdrs, tag) in enumerate(workloads):
            indexes = [headers_hash[hdr][index] if index in headers_hash[hdr] else -1 for hdr in valid_hdrs]
            k_data = group_data[k]
...
def CombineData(args):
    ...
    for index, workload in enumerate(workloads):
        ...
        indexes = [common_hdrs[hdr][index] if index in common_hdrs[hdr] else -1 for hdr in hdrs]
    ...
    for index in range(xdata.size):
        if xdata[index] == best_point[0]:
            ...
            if len(pareto_point[1])==0 or best_point[1] < prev_best_point[1]:
                is_best = True
            if len(pareto_point[1])==0 or best_point[1] < (prev_best_point[1] + prev_best_point[1] * deviation):
                is_second_best = True
...
def PlotGraph(args, group_data, x_axis_index, y_axis_index):
    ...
    def GetColumn(data, cols):
        ...
        def IsValidColumn(data_line, cols):
            ...
            if '1e+18' in data_at_indexes:
                return False
            return True
        pruned_data = [data_line for data_line in data if IsValidColumn(data_line, cols)]
    ...
    for key in sorted(graph_data.keys()):
        ...
        if ax != None:
            ax.bar(x, y, zs=key, width=100.0, zdir='y', color=colors[color_index%len(colors)], alpha=0.8)
    ...
    if ax != None:
        ...
        if args.legend_title != '':
            ...
            plt.legend(title=args.legend_title, loc=legend_pos, ncol=int(args.legend_ncol), fontsize=12)
    if args.xticks_rotation != '':
        if args.xdatatype == 'str':
            plt.xticks(ticks=sorted(xtick_labels.keys()), rotation=int(args.xticks_rotation))
        else:
...
def GetNormalizedData(workload):
    data = workload.Get2DDataWithNormalization(workload.param_indexes + workload.cost_indexes, workload.GetData(), {})
    ...
    if fields:
        ...
        if operator == '==':
            if const_fields:
                ...
                new_data = [rdata for rdata in data if rdata[src0_index] == rdata[src1_index]]
        elif operator == '<=':
            if const_fields:
                new_data = [rdata for rdata in data if float(rdata[src0_index]) <= float(src1_index)]
            else:
                new_data = [rdata for rdata in data if float(rdata[src0_index]) <= float(rdata[src1_index])]
        elif operator == '<':
            if const_fields:
                new_data = [rdata for rdata in data if float(rdata[src0_index]) < float(src1_index)]
            else:
...
def PerformCondition(args, workload):
    ...
    if comb_re.search(condition):
        ...
        for index in range(1, len(sp), 2):
            if sp[index] == '&&':
                print("Logic condition on : -{}- and -{}-".format(sp[index-1], sp[index+1]))
                workload = GetSimpleConditionWorkload(args, workload, sp[index-1])
                workload = GetSimpleConditionWorkload(args, workload, sp[index+1])
            elif sp[index] == '||':
                print("Logic condition on : {} or {}".format(sp[index-1], sp[index+1]))
...
def InitializeWorkloadArgParse(parser):
    ...
    parser.add_argument('-common-columns', dest='only_common_columns', action='store_true')
    ...
    parser.add_argument('-sequencing', nargs="*", action='store', dest='sequencing', default="")
    ...
    parser.add_argument('-ycol', nargs="*", action='store', dest='ycol', default="cpu_cycles")
